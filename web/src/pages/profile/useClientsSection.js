...
export function useClientsSection({
  ...
  const validateClientEditForm = useCallback((form) => {
    ...
    if (tgMail) {
      const isEmail = EMAIL_REGEX.test(tgMail);
      const isTelegram = TELEGRAM_USERNAME_REGEX.test(tgMail);
      if (!isEmail && !isTelegram) {
        errors.tgMail = "Enter valid Telegram username or email.";
      } else if (tgMail.length > 96) {
  ...
  const handleClientCreateSubmit = useCallback(async (event) => {
    ...
    const payload = {
      firstName: String(clientCreateForm.firstName || "").trim(),
  ...
  const handleClientEditSave = useCallback(async (id) => {
    ...
    try {
      ...
      if (canReadClients) {
        await loadClients(clientsPage, { force: true });
      }
    } catch {
  ...
  const updateClientsSearch = useCallback((value) => {
    const nextValue = String(value || "");
    clientsSearchRef.current = nextValue;
