...
test("appointments routes expose stable contract", async () => {
  ...
  assert.deepEqual(toRouteSignatures(recorder.routes), [
    ...
    "POST /schedules/repeat-conflicts/preview",
    "POST /work-schedule",
    "PUT /breaks",
  ...
  const schedulesRepeatPreviewPost = findRoute(recorder.routes, "POST", "/schedules/repeat-conflicts/preview");
  assert.equal(typeof schedulesRepeatPreviewPost?.options?.schema?.body, "object");
...
test("finance routes expose stable contract", async () => {
  ...
  assert.deepEqual(toRouteSignatures(recorder.routes), [
    ...
    "POST /cashier/appointments/:id/ticket-discount-preview",
  ...
  const clientDepositPost = findRoute(recorder.routes, "POST", "/client-balances/deposit");
  assert.equal(typeof clientDepositPost?.options?.schema?.body, "object");

  const clientRefundPost = findRoute(recorder.routes, "POST", "/client-balances/refund");
  assert.equal(typeof clientRefundPost?.options?.schema?.body, "object");
  ...
  const appointmentDiscountPreviewPost = findRoute(recorder.routes, "POST", "/cashier/appointments/:id/ticket-discount-preview");
  assert.equal(typeof appointmentDiscountPreviewPost?.options?.schema?.params, "object");
  assert.equal(typeof appointmentDiscountPreviewPost?.options?.schema?.body, "object");
